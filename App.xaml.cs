...
using FoldR.Services;

namespace FoldR
{
    public partial class App : Application
    {
        private const string MutexName = "FoldR_SingleInstance_Mutex_v3";
        private const string EventName = "FoldR_ActivateEvent_v3";
                ...
                using (var key = Microsoft.Win32.Registry.CurrentUser.OpenSubKey(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Run", true))
                {
                    if (key != null)
                    {
                        if (startWithWindows)
                        {
                            ...
                            key.SetValue("FoldR", $"\"{exePath}\"");
                        }
                        else
                        {
                            key.DeleteValue("FoldR", false);
                        }
                    }
                }
            }
